...
fn check_when(captures: &regex::Captures, when: &[LogicalValue], static_reference: &HashMap<String, String>) -> Result<bool, Error> {
    ...
    for value in when.iter() {
        match value {
            ...
            LogicalValue::Literal(value) => stack.push(value.as_str()),
            LogicalValue::Reference(index) => {
                let name = format!("x{}", index);
                let match_str = captures.name(&name).ok_or_else(|| Error::OutOfReferenceIndex(*index))?;
                
                stack.push(match_str.as_str());
            },
            LogicalValue::Original => {
                stack.push(static_reference.get("@@").unwrap());
            },
            LogicalValue::NowForm => {
                stack.push(static_reference.get("@%").unwrap());
            },
            LogicalValue::Equal => {
                if stack.len() < 2 {
